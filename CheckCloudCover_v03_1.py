    ...
    with rio.open(inRaster) as rioRaster:
        ...
        for i in range(len(projGDF)):
            ...
            try:
                ...
                if np.unique(out_image).all() != 255:
                    if args:
...
def extract_zip(inZipFile, outFolder, files = None):
    '''
    ...
    if not files:
        ...
        zip_ref.extract(member = files, path = outFolder)
    zip_ref.close()
...
def main():
    ...
    if wildcard:
        ...
        imageArchive = glob.glob(os.path.join(ImageDirectory, "{}*".format(imageIdentifier[platform])))
    print("Image directory is {}".format(ImageDirectory))
    print('Found {} products in archive'.format(len(imageArchive)))
    ...
    for image in imageArchive:
        ...
        if os.path.isdir(image):
            ...
            if platform == "Sentinel2Theia":
                targetFiles = [os.path.basename(image).replace('.zip', '/MASKS/{}.tif').format(os.path.basename(image).replace(".zip", extension)) for extension in ["_CLM_R1", "_EDG_R1"]]
                for targetFile in targetFiles:
                    extract_zip(image, tempDirectory, files=targetFile)
            else:
        ...
        featureOverlay = extract_by_feature(cloudRaster.tempRaster, inFeatures, identifier, 1) 
...
# =============================================================================
    ...
    print('Running tool for {} Products'.format(platform))
